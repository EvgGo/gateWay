...
func ConnectWithRetry(ctx context.Context, host string, port int, cfg config.DialConfig, log *slog.Logger) (*grpc.ClientConn, error) {
	...
	if cfg.UseTLS {
		// ВАЖНО: если host = IP (127.0.0.1), TLS может упасть из-за ServerName/SAN.
		// Обычно для TLS используют доменное имя (например localhost) и сертификат с SAN=localhost.
		tlsCfg := &tls.Config{
